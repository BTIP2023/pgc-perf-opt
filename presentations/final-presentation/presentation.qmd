---
title: "Performance Evaluation and Optimization of an Unsupervised Machine Learning Workflow for Discriminating Major COVID-19 Variants of Concern in the Philippines"
author: "<b>Project Lead:</b> Mark Joseph Garrovillas, RCh </br> <b>Interns:</b> Brylle Joshua Logroño, John Matthew Felices, Yenzy Urson Hebron"
date: "July 28 2023"
institution: "Core Facility for Bioinformatics"
format: 
  revealjs:
    date-format: long
    slide-number: c/t
    width: 1920
    height: 1080
    theme: [default, custom.scss]
    navigation-mode: vertical
    controls-layout: bottom-right
    controls-tutorial: true
    preview-links: auto
    slide-level: 3
    multiplex: true
    chalkboard: 
      boardmarker-width: 5
    search:
    transition: slide
    background-transition: fade
    transition-speed: slow
title-slide-attributes:
      data-background-image: presentation_files/img/PGCTitleSlideDark.svg
      data-background-size: cover
      data-background-color: "#5E205E"
      transition: slide
      background-transition: fade
      transition-speed: slow
---

# Background of the Study {background-color="#5E205E" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="fade-in slide-out" transition-speed="slow"}

## Bioinformatics pipelines {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Linear Algebra libraries {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Processor vulnerability mitigations {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}


# Objectives of the Study {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}

## Improvement of a sample pipeline that discriminates between COVID19 variants using an unsupervised machine learning framework {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
![](presentation_files/img/journal.pone.0285719.png){.fragment fig-align="center" height=40% width=40%}

## Which linear algebra library implementation (BLAS,LAPACK,OpenCL,IntelMLK) gives the most optimal runtime for the workflow? {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
![](presentation_files/img/linear-algebra.jpg){.fragment fig-align="center" height=60% width=60%}

## Does processor vulnerability mitigations (Spectre and Meltdown) significantly affect the runtime of the workflow? {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
![](presentation_files/img/spectre_meltdown.png){.fragment fig-align="center" height=60% width=60%}

# Improved Workflow {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}

## Improved Workflow summary {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
![](presentation_files/img/pipeline-flowchart.png){.fragment fig-align="center" height=60% width=60%}

## Data Preprocessing: FASTA and Metadata files were extracted from .tar files obtained from the GISAID database. {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Data Preprocessing: A treemap that visualizes the distribution of different  {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Data Preprocessing: A stratified random sampler was utilized to obtain a smaller sample of all the sequences available in the GISAID database. {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Data Preprocessing: Entries with erroneous metadata entries were filtered from the data set to prevent issues in downstream analysis {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Data Preprocessing: Sequences and metadata of preprocessed data can be saved for possible use in further downstream analysis {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## kmer analysis: kmer frequencies can be used to represent a specific sequence for downstream profiling {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

## Dimensional-Reduction: PCA analysis is able to reduce the number of dimensions of the highly dimensional k-mer data set using linear reduction{data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

::::: {.columns}

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="Principal Components Plot for k=7"}

load(file="presentation_files/img/pca-7.RData")
p
```
::::

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="3D Principal Components Plot for k=7"}

load(file="presentation_files/img/3d-pca-7.RData")
p
```
::::

:::::


## Dimensional-Reduction: tSNE is an non-linear method that is able to reduce the number of dimensions of the highly dimensional k-mer data set {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

::::: {.columns}

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="tSNE Plot for k=7"}

load(file="presentation_files/img/tsne-7.RData")
p
```
::::

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="3D tSNE Plot for k=7"}

load(file="presentation_files/img/3d-tsne-7.RData")
p
```
::::

:::::

## Dimensional-Reduction: UMAP is a newer non-linear method that is able to reduce the number of dimensions of the highly dimensional k-mer data set {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
::::: {.columns}

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="UMAP Plot for k=7"}

load(file="presentation_files/img/umap-7.RData")
p
```
::::

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="3D UMAP Plot for k=7"}

load(file="presentation_files/img/3d-umap-7.RData")
p
```
::::

:::::

## AGNES Clustering: AGNES Clustering uses a "bottom-up" approach to group similar data points together into a dendrogram {data-="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
::::: {.columns}

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="Dendrogram grouping sequences by variant"}

load(file="presentation_files/img/clustering-variant.RData")
p
```
::::

:::: {.column width="50%"}
```{r out.width="900px", out.height="450px", fig.cap="Dendrogram grouping sequences by region"}

load(file="presentation_files/img/clustering-region.RData")
p
```
::::

:::::

# Insights from the plots generated by the pipeline {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}



# Benchmarking of the pipeline using different linear algebra libraries and processor vulnerability mitigations {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"} 

# Conclusion {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}

# Recommendations {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}

# References {background-color="#447099" data-background="presentation_files/img/PGCTitleSlideDark.svg" transition="slide-in"}

## References {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}

- Research Article:

  - <https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0285719>

- Image Sources:
  - <https://i.ytimg.com/vi/0WDhKCPtbng/maxresdefault.jpg>
  - <https://web.archive.org/web/20180620212658/https://danielmiessler.com/images/Screen-Shot-2018-01-05-at-08.29.00.png>
  - <https://www.docker.com/wp-content/uploads/2022/03/vertical-logo-monochromatic.png>
  - <https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Git-logo.svg/1280px-Git-logo.svg.png>
