...
## *Rationale 1:* Treating Philippine Data with an enhanced UML workflow based on Chandra et al. may uncover new insights about COVID-19 in the country. {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
...
For instance, hardcoded working directories and variant names (as shown in the code snippet from Chandra et al.) hinder reproducibility and prevents scaling up the project to more variants based on the data. Efficiency also suffers from running `kmer_df` once for each variant name.

Among many other reasons, an in-house and inspired yet highly original UML variant classifier workflow is in order.

## Performance Evaluation and Optimization Rationales
::: {layout-ncol=2 layout-valign="center"}
![Access to powerful computers built for bioinformatics is limited.](presentation_files/img/hpc-1.png){fig-align="center"}
![Many Filipinos only have access to these setups.](presentation_files/img/comshop-1.jpeg){fig-align="center"}
:::
...
## CRediT (Contribution Roles Taxonomy) {data-background="presentation_files/img/PGCSlideLight.svg" transition="fade-in slide-out" transition-speed="slow"}
  ...
  - Brylle Joshua Logroño: Data Curation, Formal Analysis (equal), Investigation (equal), Methodology, Software (lead), Validation (equal), Visualization
