...
The workflow is well-tested on odd $k$-mer count matrices where $k = \{3, 5, 7\}$ and with variant stratum sizes of $\textrm{strat_size}\in[1,1000]$ for the sampling. Higher values for these parameters would require larger processing power, and scaling up to the complete Philippine dataset (`country_exposure = "Philippines" && strat_size >= 25000`) is indeed an ongoing objective. Do note that [most parameters](#params) of the pipeline are highly customizable. Aside from a project-specific [Docker image](https://hub.docker.com/r/yshebron/pgc-perf-opt){target="_blank"}, the pipeline's reproducibility were also guaranteed by making the source code adapt to any R 4.3.1 environment.

All paths used and discussed are relative to the project root, which is given by the directory where the project is cloned into or where `pgc-perf-opt.Rproj` resides (`getwd()="`r getwd()`"`). For the codebase, see `code`. For the data sources, overviews, and artifacts, see `data`. For the outputs, see `results` and `presentations`.

***

# Primer on Data Formats {#dformat .tabset}

Before discussing the actual pipeline, one must first be familiar with the data that will be processed. The data will undergo the following transformations within the simplified data pipeline below:
...
We discuss the most relevant are 

## GISAID Data

## 

With these in mind, we may now proceed to the setup. Guide
