...
This notebook tackles the main code in `code/pipeline.R`, guiding us through the workflow's setup and execution, along with a brief exploration of the results.
...
* `plotly`: used together with `ggplot2` to produce interactive results.
* `htmlwidgets`: used for saving HTML results.
* `ape`: used for reading and writing FASTA files in `DNAbin` format.
* `seqinr`: an `ape` alternative, reads and writes FASTA files in `char` format.
* `kmer`: used to generate $k$-mer counts, works more efficiently with `DNAbin`.
* `validate`: used for the `%vin%` membership operator.
* `gsubfn`: used to destructure more than one return value.
* `Rtsne`, `tsne`, `umap`: used for accessing dimensionality reduction algorithms.
* `ggdendro`, `dendextend`: used to produce 
* `devtools`: supports `install_github` for installing `ggbiplot`.
* `microbenchmark`: for the benchmarking side objectives.
* `highcharter`: used for some intermediate results such as the treemaps.

## B. Load Sources
If this file is loaded in RStudio with the corresponding `.Rproject`, then the root is the directory where the `.Rproject` file can be found. Unfortunately, RMarkdown files do not respond to `.Rproject`, and hence still works on its own directory space, necessitating the use of  relative paths in the following sources.
```{r}
...
## C. Set Parameters
Pipeline parameters can be set in this section. These parameters will control the flow and behavior of the pipeline.

### Pipeline General Parameters
These parameters are shared by multiple subroutines.
...
* `stamp` (`str|NULL`): the suffix added to files generated by the preprocess subroutines.
  ...
  + Any stamp is valid, but the recommended stamps are "`str(strat_size)`" (same string as `strat_size` value, to be discussed later), "`get_time()`", and `NULL` or "" (empty string) for no stamp.
* `write_fastacsv` (`bool`): controls writing of intermediate files.
* `kmer_list` (`c(int)`): list of kmer values to evaluate.
  + Default in `pipeline.R` is `c(3, 5, 7)` but not in this document, where only k-value `7` will be the focus of evaluations.
* `strat_size` (`int`): number of samples randomly selected per stratum.
  ...
  + Valid `strat_size` will only be those with corresponding files in both `data/interm` and `data/kmers`.
